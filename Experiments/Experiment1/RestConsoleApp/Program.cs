...
class Program
{
    private static ConcurrentBag<long> _results;
    ...
    static async Task Main()
    {
        ...
        await Test(client, uri, stopwatch);
        Thread.Sleep(1000);
        _results = new ConcurrentBag<long>();
        ...
        for (int i = 0; i < 100; i++)
        {
            tasks.Add(Task.Run(() => Test(client, uri, stopwatch)));
        }
    ...
    private static async Task Test(HttpClient client, string uri, Stopwatch stopwatch)
    {
        stopwatch.Restart();
